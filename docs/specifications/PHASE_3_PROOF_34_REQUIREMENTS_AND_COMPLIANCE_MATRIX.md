...
| R34-064 | Test runner | Focused tests and complete repository suite must pass using repository test convention. | `scripts/run-test-suite.sh` exists and separates migration/full suite with its test-safety guard. | Current foundation focused tests pass 19/19; guarded suite passes 31 migration and 430 non-migration tests. Future Proof 34 runtime/UI tests remain acceptance work. | **CURRENT SUITE PASS / FUTURE TESTS PENDING** |
| R34-065 | Schema | Alembic current/check and schema drift zero. | No new migration yet. | Upgrade/current, constraints/triggers, empty and retained-history downgrade, one head, and Alembic zero-drift check pass for the foundation. | **FOUNDATION SCHEMA PASS** |
