...
| R34-046 | Health read model     | UI/API cannot infer robots reason by combining unrelated run statuses, exceptions and timestamps.                                                                                                                  | 34C projects immutable robots evidence, exact gates, and the same non-consuming Owner-policy context used by runtime through `RobotsGuiService`.                            | Robots health state must come from explicit server-side operation/evidence/policy projection.                                                                                                                                                                                           | **34C IMPLEMENTED CANDIDATE**                           |
| R34-047 | Health UI             | Compact robots summary relative to the publisher.                                                                                                                                                                  | 34C shows green `Allows`, red enforced `Disallows` plus `Override`, green overridden `Disallows` plus accessible `Owner override active`, unavailable reason, stale, and not-checked states. | Admin/Owner detail exposes phase, reason/code, HTTP status, retryability, sanitized summary, evidence, action, and history.                                                                                                                                                | **34C IMPLEMENTED CANDIDATE / OWNER REVIEW PENDING**    |
| R34-048 | Detail UI             | Dedicated Owner review view.                                                                                                                                                                                       | Authenticated administrative detail shows effective decision, external evaluation, UA/rule, retrieval/parser provenance, policy chain, exact gate, scope preview entry, and retained histories. | Complete Owner visual/device review in the acceptance sequence.                                                                                                                                                                                   | **34C IMPLEMENTED CANDIDATE / OWNER REVIEW PENDING**    |
| R34-049 | UI access             | The Proof 34 GUI must expose the Owner's existing robots override authority through the shared ledger; missing UI controls do not narrow Owner authority.                                                           | 34C uses server-derived identity, `owner.policy`, CSRF, fresh passkey reauthentication, reason/risk/scope confirmation, exact evidence/policy fingerprints, append-only audit, and every applicable registered scope. | Complete Owner workflow review; no separate Proof 34 authority store exists.                                                                                                                          | **34C IMPLEMENTED CANDIDATE / OWNER REVIEW PENDING**    |
...
| R34-051 | Date/time UI          | Time hidden by default; dates use American UI standard; current year omitted; time only when necessary and 12-hour.                                                                                                | New 34C fields use the shared `datetime_user` filter; compact next-review display is date-only.                                                                             | Confirm rendering during Owner/device acceptance.                                                                                                                                                                                                                                        | **IMPLEMENTED CANDIDATE / UI REVIEW PENDING**           |
| R34-052 | UI governance         | Modified reusable Endpoint Health component must have governing component record/version/review.                                                                                                                   | Registry version 2 adds CMP-0010 with ownership, variants, states, dependencies, device boundary, and review limitation.                                                    | Complete formal acceptance review.                                                                                                                                                                                                                                                       | **COMPONENT RECORD PASS / REVIEW PENDING**              |
| R34-053 | UI governance         | Materially changed workflow requires prototype/review and permanent acceptance evidence.                                                                                                                           | 34C implementation candidate and component record exist; no Owner acceptance record is issued before review.                                                               | Owner reviews desktop/laptop/tablet/mobile adaptation, accessibility, date-time behavior, and mutation workflow; record the resulting UAR.                                                                                                                                                 | **OWNER REVIEW / UAR PENDING**                          |
...
| R34-061 | Tests | UI and worker resolve the same external finding and effective decision; viewing must not mutate/consume. | Focused 34C tests exercise the runtime-created evidence through the GUI projection and mutation flow, including all approved badge states and non-consuming preview. | Repeat in complete 34D acceptance inventory and live proof. | **34C FOCUSED PASS / 34D PENDING** |
| R34-062 | Tests | Stale UI preview must fail. | Service and web mutations reject changed policy basis; 34C additionally binds confirmation to endpoint, exact evaluation/snapshot, decision freshness, policy basis, and scope. | Repeat in complete 34D acceptance inventory. | **34C FOCUSED PASS / 34D PENDING** |
...
| R34-069 | Owner information access | Robots unavailable phase, reason, status, retryability, summary, evidence, and history are internal **and** Owner information. | 34C preserves the explicit classification and exposes the registered structured information in the authenticated Owner/Admin detail view. | Confirm the presentation during Owner/device acceptance; never narrow the underlying access obligation. | **34C IMPLEMENTED CANDIDATE / OWNER REVIEW PENDING** |
...
- R34-057 through R34-063 now have backend/runtime or focused 34C evidence as
  applicable. R34-061 and R34-062 pass the focused authenticated GUI/runtime
  slice without view-time consumption; complete-suite repetition remains 34D.
- R34-046 through R34-052 are implemented candidates in 34C. R34-053 remains
  open for Owner visual/device review and the permanent UI acceptance record.
...
**Proof 34 status: 34C AUTHENTICATED GUI IMPLEMENTED CANDIDATE — OWNER UI REVIEW AND 34D FINAL ACCEPTANCE PENDING**

The restart requirements remain the controlling acceptance ledger. The 34B
and 34C change reports record runtime and authenticated GUI implementation
evidence without claiming Owner UI acceptance or final live acceptance work.
...
- UI/worker policy parity proof: PASS in focused 34C slice; complete 34D repeat pending
- UI acceptance record: PENDING OWNER REVIEW
- 34B backend change report: PASS;
  `docs/change-reports/PHASE_3_PROOF_34B_ROBOTS_RUNTIME_IMPLEMENTATION.md`
- 34C authenticated GUI change report: IMPLEMENTED CANDIDATE;
  `docs/change-reports/PHASE_3_PROOF_34C_AUTHENTICATED_ROBOTS_GUI_AND_OWNER_OVERRIDE.md`
...
**Current disposition:** NOT FINALLY ACCEPTED — 34A, 34A.1, 34B, AUTHENTICATION FOUNDATION, AND 34C IMPLEMENTED CANDIDATES; OWNER UI REVIEW AND 34D ACCEPTANCE REMAIN
