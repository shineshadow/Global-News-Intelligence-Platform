...
There are **six concrete values plus two architectural decisions that must not be invented during implementation**.

| Decision                                                                                   | Current authoritative state                                                                                                | Required before implementation                                          |
| ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `acquisition.robots.cache.max_age_seconds` default                                         | Not fixed by standard                                                                                                      | **Owner approval**                                                      |
| `acquisition.robots.cache.max_stale_seconds` default                                       | Not fixed by standard                                                                                                      | **Owner approval**                                                      |
| `fetch_limits.max_response_bytes` default                                                  | Not fixed                                                                                                                  | **Owner approval**                                                      |
| `fetch_limits.max_redirects` default                                                       | Not fixed                                                                                                                  | **Owner approval**                                                      |
| `fetch_limits.connect_timeout_seconds` default                                             | Not fixed                                                                                                                  | **Owner approval**                                                      |
| `fetch_limits.read_timeout_seconds` default                                                | Not fixed                                                                                                                  | **Owner approval**                                                      |
| Robots parser + exact version/supply-chain pin                                             | Standard requires a registered/pinned parser and trace, but does **not** select one at this reset                          | **Fresh proposal + Owner approval**                                     |
| Mediated adapters — RSSHub/RSS-Bridge/changedetection/Playwright publisher-robots boundary | Existing adapter standards do not define how exact publisher robots authority is conveyed through intermediary acquisition | **Architecture decision/acceptance before those paths are implemented** |
...
| `acquisition.robots.crawl_delay.enforce`     | boolean; default `true` when supported  | global, adapter, platform, credential, origin, Source, endpoint, request | Scheduling after robots evaluation                      |
| `acquisition.robots.cache.max_age_seconds`   | bounded positive integer                | global, adapter, platform, origin, Source, endpoint                      | Calculating `fresh_until`                               |
| `acquisition.robots.cache.max_stale_seconds` | bounded nonnegative integer             | global, adapter, platform, origin, Source, endpoint                      | Calculating `stale_until`                               |
| `acquisition.robots.fetch_limits`            | closed versioned object                 | global, adapter, platform, origin, Source, endpoint                      | Before robots retrieval                                 |
...
| R34-007 | Policy registration   | Every Owner-controllable key requires a registered definition, type, validation schema, default, scopes, resolution point, restart requirement, consequences, evidence and tests.                                  | No complete registered-definition catalog exists. Current service accepts policy key/value/default from callers.                                                           | Add authoritative policy registry. Arbitrary caller defaults cannot control resolution; mismatches must fail.                                                                                                                                                                           | **GAP**                                                 |
| R34-008 | Policy service        | `resolve(...)` must provide actual runtime value.                                                                                                                                                                  | Exists as foundation.                                                                                                                                                      | Refactor to registered definitions while retaining single precedence implementation.                                                                                                                                                                                                    | **FOUNDATION — EXTEND**                                 |
| R34-009 | Policy service        | `explain(...)` must expose the complete decision without consuming authority.                                                                                                                                      | Not implemented.                                                                                                                                                           | Implement non-consuming explanation DTO/service; prove one-use override remains untouched by views, health, API and CLI inspection.                                                                                                                                                     | **GAP**                                                 |
| R34-010 | Policy service        | `preview_override(...)` must evaluate hypothetical authority without persistence/consumption.                                                                                                                      | Not implemented.                                                                                                                                                           | Implement current/proposed decision, winning/rejected scope, affected subjects/gates/operations and consequences.                                                                                                                                                                       | **GAP**                                                 |
| R34-011 | Stale preview         | Preview requires deterministic `basis_fingerprint`; mutation must transactionally reject stale basis.                                                                                                              | Not implemented.                                                                                                                                                           | Implement fingerprint and stable `owner_policy.preview_stale` rejection.                                                                                                                                                                                                                | **GAP**                                                 |
...
| R34-013 | Robots policies       | `acquisition.robots.enforce` registered boolean default `true`.                                                                                                                                                    | Existing foundation/default `true`; worker resolves it.                                                                                                                    | Register under full Proof 34 definition and use it only to control GNI enforcement, never external evidence.                                                                                                                                                                            | **FOUNDATION — EXTEND**                                 |
| R34-014 | Robots policies       | `acquisition.robots.unavailable_action` enum `delay/allow/deny`, default `delay`.                                                                                                                                  | Absent.                                                                                                                                                                    | Register and resolve at unavailable-evidence decision point with complete permitted scope family.                                                                                                                                                                                       | **GAP**                                                 |
| R34-015 | Robots policies       | `acquisition.robots.crawl_delay.enforce` default `true` **if the selected registered parser supports Crawl-delay**.                                                                                                | Absent. Parser is not selected at reset.                                                                                                                                   | Register only after approved parser capability is established; observed Crawl-delay and effective enforcement remain separate.                                                                                                                                                          | **BLOCKED BY PARSER DECISION**                          |
| R34-016 | Robots policies       | `acquisition.robots.cache.max_age_seconds`.                                                                                                                                                                        | Absent; governing standard deliberately leaves default for Owner review.                                                                                                   | Owner must explicitly approve repository default before implementation is marked conformant.                                                                                                                                                                                            | **OWNER APPROVAL REQUIRED**                             |
| R34-017 | Robots policies       | `acquisition.robots.cache.max_stale_seconds`.                                                                                                                                                                      | Absent; default unresolved by governing standard.                                                                                                                          | Owner must explicitly approve repository default before implementation.                                                                                                                                                                                                                 | **OWNER APPROVAL REQUIRED**                             |
| R34-018 | Robots policies       | `acquisition.robots.fetch_limits` closed versioned object with response bytes, redirects, connect timeout and read timeout.                                                                                        | Absent; default unresolved.                                                                                                                                                | Owner must explicitly approve all four default values. Unknown/missing/out-of-range fields and wrong types must fail.                                                                                                                                                                   | **OWNER APPROVAL REQUIRED**                             |
...
| R34-021 | Persistence           | Authoritative immutable robots snapshots.                                                                                                                                                                          | No `acquisition_robots_snapshots` table/model.                                                                                                                             | Add strongly constrained snapshots with retrieval state, HTTP evidence, cache bounds, validators, parser provenance, warnings/digest and immutable history.                                                                                                                             | **GAP**                                                 |
| R34-022 | Persistence           | Authoritative immutable exact robots evaluations.                                                                                                                                                                  | No `acquisition_robots_evaluations`.                                                                                                                                       | Persist exact snapshot + endpoint/request/user-agent/path/matched-rule evidence and external decision.                                                                                                                                                                                  | **GAP**                                                 |
| R34-023 | Persistence           | Exact robots gates.                                                                                                                                                                                                | No `acquisition_robots_gates`. Generic rate buckets contain `robots_disallow_until`.                                                                                       | Add exact authoritative gate/history model. Generic rate state may reference robots records but cannot itself be robots authority.                                                                                                                                                      | **GAP**                                                 |
...
| R34-029 | Parser                | Parser must be deliberately selected, pinned/versioned and have decision-trace provenance.                                                                                                                         | **No governing parser/version is approved at reset.**                                                                                                                      | Fresh parser proposal and Owner approval are required. The removed Protego choice is **not** authority. Parser/trace disagreement must not be guessed into allow/deny.                                                                                                                  | **OPEN DESIGN / OWNER APPROVAL**                        |
| R34-030 | Crawl-delay           | Valid observed Crawl-delay may govern scheduling; malformed Crawl-delay must not corrupt otherwise valid Allow/Disallow evidence.                                                                                  | Not implemented.                                                                                                                                                           | Implement only after parser decision; combine effective delay with other independent eligibility controls.                                                                                                                                                                              | **GAP AFTER PARSER DECISION**                           |
...
| R34-034 | Unavailable           | Trustworthy robots evidence unavailable.                                                                                                                                                                           | No explicit robots unavailable model.                                                                                                                                      | `delay` installs exact unavailable gate + revalidation; `allow` proceeds retaining unavailable evidence; `deny` blocks as a **GNI policy decision**, not external denial.                                                                                                               | **GAP**                                                 |
...
| R34-040 | Mediated adapters     | Robots evidence must describe the **publisher request**, not GNI's internal intermediary call.                                                                                                                     | RSSHub/RSS-Bridge and changedetection/Playwright adapters call installation-owned intermediaries. Existing specs do not define Proof 34 publisher-robots evidence handoff. | Approve an explicit architecture: GNI evaluates publisher target before intermediary invocation **or** a separately approved intermediary evidence contract provides exact trustworthy publisher robots evidence. Internal service `robots.txt` cannot stand in for publisher evidence. | **OPEN ARCHITECTURE BLOCKER**                           |
...
| R34-044 | OwnerOperationResult  | Required robot outcomes/reasons/versioned detail schemas.                                                                                                                                                          | Absent.                                                                                                                                                                    | Register allowed/disallowed/unavailable/unreachable/stale/not-enforced semantics and bounded details; unregistered strings fail.                                                                                                                                                        | **GAP**                                                 |
...
| R34-047 | Health UI             | Compact robots summary in endpoint health row.                                                                                                                                                                     | Not present.                                                                                                                                                               | Add `Robots: Allowed / Disallowed / Unavailable / Stale / Not checked`, enforcement state, next review date and `Review decision`.                                                                                                                                                      | **GAP**                                                 |
...
| R34-049 | UI mutation           | First Owner-facing UI must be read-only for override mutation.                                                                                                                                                     | Current generic web app lacks the required authenticated Owner mutation safeguards.                                                                                        | No browser creation/supersession/revocation. It may provide explanation/preview and exact validated CLI path.                                                                                                                                                                           | **MANDATORY DESIGN**                                    |
...
| R34-054 | Migration             | Add strongly constrained robots persistence from current head.                                                                                                                                                     | No robots evidence/gate tables at reset. Recursive tree confirms absence.                                                                                                  | Migration revises `a9c1e3f5b7d2`; models/import metadata/docs updated coherently.                                                                                                                                                                                                       | **GAP**                                                 |
| R34-055 | Migration             | Historical robots evidence must not be silently destroyed by downgrade.                                                                                                                                            | Repository already uses lossless-only downgrade refusal for Owner-policy/archive history.                                                                                  | Robots migration must refuse downgrade when retained history would be lost, as required by Proof 34 standard.                                                                                                                                                                           | **GAP / PATTERN EXISTS**                                |
...
| R34-057 | Tests | Policy precedence/default/override/explain/preview/staleness/non-consumption. | Existing owner-policy tests cover foundation precedence, expiry/revoke/single-use, but not registry/explain/preview/fingerprint. | Add full policy-decision-context test matrix. | **GAP** |
...
| R34-059 | Tests | Cache/revalidation/unavailable/Crawl-delay/parser/evidence tests. | None. | Prove 304 linkage, expired evidence, malformed/unreachable/stale handling, body/redirect limits, trace provenance and Crawl-delay disposition. | **GAP** |
...
| R34-061 | Tests | UI and worker resolve same effective decision; viewing must not mutate/consume. | No decision UI/service exists. | Cross-layer parity test required. | **GAP** |
| R34-062 | Tests | Stale UI preview must fail. | No preview. | Test basis change between preview and attempted mutation. | **GAP** |
...
| R34-064 | Test runner | Focused tests and complete repository suite must pass using repository test convention. | `scripts/run-test-suite.sh` exists and separates migration/full suite with its test-safety guard. | Run Proof 34 focused tests plus migration tests plus complete guarded suite. | **ACCEPTANCE REQUIRED** |
| R34-065 | Schema | Alembic current/check and schema drift zero. | No new migration yet. | Prove upgrade, current, check, constraints/indexes/triggers and zero drift. | **ACCEPTANCE REQUIRED** |
...
| R34-068 | Completion | Proof 34 cannot be declared complete until policy registration/default approval, DB, retrieval/cache/parser, evaluations/gates, Owner context/results, read-only UI, tests, runtime proof and report are complete. | Governing standard explicitly says Proof 34 is incomplete at reset. | All rows above that are implementation/acceptance obligations must close. | **NOT COMPLETE** |
...
### CONTROL-07 — Read-Only Browser Policy Control for Proof 34
...
The initial Proof 34 browser interface must not create, supersede, revoke, or otherwise mutate Owner-policy authority.

Browser mutation requires the separate authentication, authorization, CSRF, reauthentication, confirmation, stale-preview, and transactional safeguards defined by the governing Owner-policy standard.
...
**Implementation is not yet authorized to begin.**

The repository review is complete, but the following decisions remain unresolved:

1. `acquisition.robots.cache.max_age_seconds` default
2. `acquisition.robots.cache.max_stale_seconds` default
3. `fetch_limits.max_response_bytes` default
4. `fetch_limits.max_redirects` default
5. `fetch_limits.connect_timeout_seconds` default
6. `fetch_limits.read_timeout_seconds` default
7. Robots parser and exact parser version
8. Robots-authority architecture for mediated acquisition paths including RSSHub, RSS-Bridge, changedetection, and Playwright

These decisions require Owner approval or architectural acceptance before their affected implementation work begins.
...
- unresolved Owner defaults are explicitly approved;
- parser selection/version is explicitly approved;
- the mediated-adapter robots boundary is resolved;
...
**Current disposition:** NOT ACCEPTED — IMPLEMENTATION HAS NOT BEGUN
